...
def tokenize_and_mask(text, span_length, pct, ceil_pct=False):
    # code Llama masking
    if args.mask_filling_model_name.startswith('CodeLlama'):
        mask_string = '<FILL_ME>'
        # code perturbation method
        if args.code_perturbation_method == 'token':
            tokens = text.split(' ')

            # setting 15% of the tokens
            span_length = pct * len(tokens) / (args.buffer_size * 2)
            if ceil_pct:
                span_length = np.ceil(span_length)
        ...
        elif args.code_perturbation_method == 'line':
            tokens = text.split('\n')
            ...
            text = '\n'.join(tokens)
        elif args.code_perturbation_method == 'feature':
            tokens = None
            text = ''
    # else
    else:
...
# setting codeLlama for mask model
def replace_masks(texts):
    if args.mask_filling_model_name.startswith('CodeLlama'):
        mask_tokenizer.pad_token_id = mask_tokenizer.eos_token_id
        input_ids = mask_tokenizer(texts, return_tensors="pt", truncation=True, padding=True, max_length=512).to(DEVICE)
        ...
        result = mask_tokenizer.batch_decode(outputs[:, len(input_ids['input_ids'][0]):], skip_special_tokens=True)
        result = [text.replace("<FILL_ME>", filling) for text, filling in zip(texts,  result)]
    else:
        n_expected = count_masks(texts)
        stop_id = mask_tokenizer.encode(f"<extra_id_{max(n_expected)}>")[0]
        tokens = mask_tokenizer(texts, truncation=True, return_tensors="pt", padding=True).to(DEVICE)
        outputs = mask_model.generate(**tokens, max_length=150, do_sample=True, top_p=args.mask_top_p, num_return_sequences=1, eos_token_id=stop_id)
        result = mask_tokenizer.batch_decode(outputs, skip_special_tokens=False)
    return result
...
def apply_extracted_fills(masked_texts, extracted_fills, skip=False):
...
def perturb_texts_(texts, span_length, pct, ceil_pct=False):
    if not args.random_fills:
        ...
        if args.mask_filling_model_name.startswith('CodeLlama'):
            perturbed_texts = replace_masks(masked_texts)
        else:
            raw_fills = replace_masks(masked_texts)
            extracted_fills = extract_fills(raw_fills)
            perturbed_texts = apply_extracted_fills(masked_texts, extracted_fills)
        # print('original text :', texts[0])
        # print('masked text :', masked_texts[0])
        # print('raw fills :', raw_fills[0])
        # print('perturbed text :', perturbed_texts[0])
        ...
        while '' in perturbed_texts:
            ...
            new_perturbed_texts = apply_extracted_fills(masked_texts, extracted_fills, skip)
        ...
        if args.random_fills_tokens:
            ...
            tokens = base_tokenizer(texts, truncation=True, return_tensors="pt", padding=True).to(DEVICE) #fixed
...
def perturb_texts(texts, span_length, pct, ceil_pct=False):
    ...
    if '11b' in mask_filling_model_name:
        chunk_size //= 2
    elif '7b' in mask_filling_model_name:
        chunk_size //= 2
...
def sample_from_model(texts, min_words=55, #FIXED
                       prompt_tokens=30):
...
def get_ll(text):
    if args.openai_model:        
        ...
        with torch.no_grad():
            tokenized = base_tokenizer(text,  truncation=True, return_tensors="pt").to(DEVICE) # FIXED
...
def get_roc_metrics(real_preds, sample_preds):
    fpr, tpr, _ = roc_curve([0] * len(real_preds) + [1] * len(sample_preds), real_preds + sample_preds) #TODO: best threshold
    roc_auc = auc(fpr, tpr)
...
def run_perturbation_experiment(results, criterion, span_length=10, n_perturbations=1, n_samples=500):
    ...
    last_predictions_sample = 0
    for i in range(0, len(predictions['real'])):
        if np.isnan(predictions['real'][i]) or np.isnan(predictions['samples'][i]):
            predictions['real'][i] = last_predictions_real
            predictions['samples'][i] = last_predictions_sample
            print('isNaN')
        else:
            last_predictions_real = predictions['real'][i]
            last_predictions_sample = predictions['samples'][i]
...
def run_baseline_threshold_experiment(criterion_fn, name, n_samples=500):
    ...
    for x in results:
        if math.isnan(x['original_crit']):
            print('error in original crit', x)
        if math.isnan(x['sampled_crit']):
            print('error in sampled crit', x)
...
def generate_samples(raw_data, batch_size):
    ...
    for batch in range(len(raw_data) // batch_size):
        ...
        sampled_text = sample_from_model(original_text, min_words=20 if args.dataset in ['code-custom'] else 55)
...
def generate_data(dataset, key):
    data = []
    if dataset == 'code-custom':
        file_path = "data/elice/elice_dataset_v3.json" 
...
def load_base_model_and_tokenizer(name):
    ...
    base_tokenizer = transformers.AutoTokenizer.from_pretrained(name, **optional_tok_kwargs, cache_dir=cache_dir, padding_side='left')
...
def run(text):
    ...
    parser.add_argument('--perturb_code', action='store_true')
    ...
    if not args.baselines_only and not args.random_fills:
        ...
        # setting code Llama for mask model
        if mask_filling_model_name.startswith('CodeLlama'):
            # setting config for CodeLlama
            model_id = "codellama/" + mask_filling_model_name + "-hf"
            mask_model = transformers.LlamaForCausalLM.from_pretrained(model_id, **int8_kwargs, **half_kwargs, cache_dir=cache_dir)
        else:
    ...
    # setting code Llama for mask tokenizer
    if mask_filling_model_name.startswith('CodeLlama'):
        model_id = 'codellama/' + mask_filling_model_name + '-hf'
        mask_tokenizer = transformers.CodeLlamaTokenizer.from_pretrained(model_id, model_max_length=n_positions, cache_dir=cache_dir)
    else:
    ...
    if args.dataset == 'custom': # custom datasets
        file_path = "data/elice/elice_dataset_v3.json" #FIXME: dataset path
        with open(file_path, 'r') as file:
            codes = json.load(file)
        machine = []
        human = []
        ...
        for code in codes:
            if code['label'] == 'human':
                human.append(code['code'])
            elif code['label'] == 'machine':
                machine.append(code['code'])

        # if machine dataset is fewer than human dataset..
        if len(machine) < len(human):
            max_idx = len(machine)
            idx=0
            while(len(machine)<len(human)):
                temp = machine[idx]
                machine.append(temp)
                idx += 1
                if idx==max_idx:
